...
class MainService:
    ...
    async def run_clusterization(self):
        ...
        if a_type == 'gender_to_age':
            ...
            result = self._clusterizationBlogic.conduct_clusterization(a_type, users_distribution)
        elif a_type == 'num_of_comments':
            age_to_comments = self._selector.users_age_to_number_of_comments()
            result = self._clusterizationBlogic.conduct_clusterization(a_type, age_to_comments)
        elif a_type == 'num_of_posts':
            age_to_posts = self._selector.users_age_to_number_of_posts()
            result = self._clusterizationBlogic.conduct_clusterization(a_type, age_to_posts)
        return  result
